...
export async function POST(req: Request, { params }: { params: Promise<{ packageId: string }> }) {
  try {
    ...
    const pkg = await trainingService.activatePackage(packageId, clientId);

    return NextResponse.json({ success: true, data: pkg });
  } catch (error: any) {
    const status = error.statusCode || 500;
    return NextResponse.json({ error: error.message || 'Failed to activate training package' }, { status });
  }
}
